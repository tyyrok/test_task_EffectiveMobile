...
def check_input_values(value, reg_type='name'):
    ...
    reg_company = re.compile('^[a-zA-Zа-яА-Я0-9-]+$') #regex for checking company name
...
def create_record(data, last_name=None, first_name=None, middle_name=None, company=None, company_phone=None, mobile_phone=None):
    ...
    if company is None:
        ...
        if check_input_values(company, 'company') is None:
            print("Название компании может состоять только из букв и цифр")
    ...
    if result:
        ...
        print("Возникла ошибка при сохранении, попробуйте еще раз")
        return menu()
...
def change_record(data):
    ...
    """
    print("""**********************************************************
    Для внесения изменений в телефонный справочник вам необходим номер записи в БД""")
    ...
    for record in data:
        if record['id'] == record_id:
            ...
            ******************************************************""")
            ...
            message_wrong_names = "Фамилия/Имя/Отчество должны состоять только из букв"
...
def find_record(data):
    ...
    ******************************************************""")
    ...
    choice = input("Выберите категорию поиска\n")
